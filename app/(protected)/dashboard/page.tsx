...
// Structural Shadcn UI Components
import {
  Card,
  CardContent,
  CardDescription,
...
import { getDecodedUser, getUserRole } from "@/utils/auth";
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Concurrent retrieval of data packets
      const [loanResponse, depositResponse] = await Promise.all([
        apiCall("/api/user_loans", "GET"),
        apiCall("/api/user_deposits", "GET").catch(() => ({
          data: { items: [] },
        })),
      ]);

      if (loanResponse?.name === "AxiosError") {
        throw new Error(
          "Unable to establish a secure connection to server ledgers.",
        );
      }

      // Safe deep parsing of underlying data maps matching payload contracts
      const fetchedLoans =
        loanResponse?.data?.items || loanResponse?.loans || [];
      const fetchedDeposits =
        depositResponse?.data?.items || depositResponse?.deposits || [];
      ...
      setErrorMessage(
        error?.message ||
          "An unanticipated system error occurred while retrieving your portfolio statistics.",
      );
    } finally {
      setLoading(false);
    }
  ...
  // Compute live portfolio aggregates derived directly from underlying state matrices
              ...
              <section className="grid gap-4 sm:grid-cols-2">
                <Card className="bg-[#241F1B] border-[#362F29] shadow-sm relative overflow-hidden group rounded-2xl">
                  ...
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-xs font-semibold text-[#8C8176] uppercase tracking-wider">
                      Active Lines of Credit
                    </CardTitle>
                    <div className="p-2 rounded-xl bg-[#1A1715] border border-[#332D28] text-[#E63946]">
                      <GiReceiveMoney className="h-4 w-4" />
                    </div>
                  </CardHeader>
                  <CardContent className="pt-1">
                    <span className="text-2xl font-bold font-mono tracking-tight text-[#FAF8F5]">
                      ₦
                      {totalLoanAmount.toLocaleString("en-NG", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                    <p className="text-[10px] text-[#8C8176] mt-1">
                      Total approved borrowing principal
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-[#241F1B] border-[#362F29] shadow-sm relative overflow-hidden group rounded-2xl">
                  ...
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-xs font-semibold text-[#8C8176] uppercase tracking-wider">
                      Approved Financing Products
                    </CardTitle>
                    <div className="p-2 rounded-xl bg-[#1A1715] border border-[#332D28] text-[#E6A15C]">
                      <VscLayersActive className="h-4 w-4" />
                    </div>
                  </CardHeader>
                  <CardContent className="pt-1">
                    <span className="text-2xl font-bold font-mono tracking-tight text-[#FAF8F5]">
                      {activeLoansCount} Active{" "}
                      {activeLoansCount === 1 ? "Contract" : "Contracts"}
                    </span>
                    <p className="text-[10px] text-[#8C8176] mt-1">
                      Verified dynamic funding facilities
                    </p>
                  </CardContent>
                </Card>
              </section>

              {/* TWO COLUMN INTERACTIVE SUBSTRUCTURE */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                {/* DOUBLE COLUMN: MAIN DATA INDEX REGION */}
                <div className="lg:col-span-2 space-y-4">
                  <Tabs defaultValue="loans" className="w-full">
                    ...
                    {/* DYNAMIC SAVINGS TAB SECTION CONTAINER */}
                    <TabsContent
                      value="deposits"
