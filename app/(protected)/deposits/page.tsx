...
export default function DepositPage() {
  ...
  const fetchDeposits = async (pageNum: number) => {
    setLoading(true);
    setErrorMessage(null);

    try {
      const response = await apiCall("/api/deposits", "GET", {
        page: pageNum,
        page_size: PAGE_SIZE,
      });
  ...
  <p className="text-xs text-[#A39990] mt-0.5">
    Review, track, and manage all customer deposit activities and balances in one place.
  </p>
</div>
